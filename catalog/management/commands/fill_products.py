...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        products_data = [
            {'name': 'iPhone 15 Pro', 'description': 'Флагманский смартфон Apple', 'price': 99999.99,
             'category': categories['Электроника']},
            {'name': 'Samsung Galaxy S24', 'description': 'Мощный Android-смартфон', 'price': 79999.50,
             'category': categories['Электроника']},
            {'name': 'Футболка хлопковая', 'description': 'Комфортная хлопковая футболка', 'price': 2499.99,
             'category': categories['Одежда']},
            {'name': 'Джинсы', 'description': 'Стильные джинсы премиум качества', 'price': 5999.99,
             'category': categories['Одежда']},
            {'name': 'Python для начинающих', 'description': 'Учебник по программированию', 'price': 1899.00,
             'category': categories['Книги']},
            {'name': 'Война и мир', 'description': 'Классика русской литературы', 'price': 1299.50,
             'category': categories['Книги']},
            {'name': 'Беспроводные наушники', 'description': 'Наушники с шумоподавлением', 'price': 8999.99,
             'category': categories['Электроника']},
            {'name': 'Фитнес-браслет', 'description': 'Умный браслет для отслеживания активности', 'price': 3999.50,
             'category': categories['Спорт']},
            {'name': 'Мяч футбольный', 'description': 'Профессиональный футбольный мяч', 'price': 2999.99,
             'category': categories['Спорт']},
            {'name': 'Кружка', 'description': 'Керамическая кружка с принтом', 'price': 999.99,
        ...
        if product_count > len(products_data):
            for i in range(len(products_data), product_count):
                ...
                self.stdout.write(f'Создан дополнительный продукт: {product.name} - {product.price} руб.')
